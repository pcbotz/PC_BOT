  ...
  #  if user_id in ADMINS: return # ignore admins
    if PM_SEARCH == True:
        ...
        await message.reply_text(
            text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/ARAKAL_THERAVAD_GROUP_02>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>",   
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("📢 𝐉𝐨𝐢𝐧 𝐑𝐞𝐪𝐮𝐞𝐬𝐭 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 📢 Join must 😒", show_alert=True)
            return
        ...
        await query.answer("⚠️ 𝗢𝗻𝗹𝘆 𝗡𝗲𝘄 𝗧𝗵𝗲𝗮𝘁𝗿𝗲 𝗥𝗲𝗹𝗲𝗮𝘀𝗲 𝗧𝗵𝗲𝗮𝘁𝗿𝗲 [𝗣𝗿𝗲𝗱𝘃𝗱] 𝗣𝗿𝗶𝗻𝘁𝘀 𝗪𝗶𝗹𝗹 𝗕𝗲 𝗔𝘃𝗮𝗶𝗹𝗮𝗯𝗹𝗲 𝗙𝗿𝗼𝗺 𝗧𝗵𝗶𝘀 𝗕𝗼𝘁 ✅", show_alert=True)

    elif query.data == "fmal":
        await query.answer("⚠️ ഈ ബോട്ടിൽ നിന്നും പുതിയതായിട്ട് തിയേറ്ററിൽ  റിലീസ് ആവുന്ന  തീയറ്റർ പ്രിന്റ്  മാത്രമേ ലഭിക്കുകയുള്ളൂ ✅", show_alert=True)
        
    elif query.data == "ftam":
        await query.answer("⚠️ புதிய தியேட்டர் ரிலீஸ் தியேட்டர் [predvd] பிரிண்ட்கள் மட்டுமே இந்த போட்டிலிருந்து கிடைக்கும் ✅", show_alert=True)

    elif query.data == "fhin":
        await query.answer("⚠️ इस बॉट से केवल नए थिएटर रिलीज़ थिएटर [predvd] प्रिंट उपलब्ध होंगे ✅", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>Tʜᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {message.from_user.mention}\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : {message.chat.title} \n\n⚠️ ഈ ബോട്ടിൽ നിന്നും പുതിയതായിട്ട് തിയേറ്ററിൽ  റിലീസ് ആവുന്ന  തീയറ്റർ പ്രിന്റ്  മാത്രമേ ലഭിക്കുകയുള്ളൂ 🗑️\n\n</b>"
        else:
            cap = f"<b>Tʜᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {message.from_user.mention}\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : {message.chat.title} \n\n⚠️ ഈ ബോട്ടിൽ നിന്നും പുതിയതായിട്ട് തിയേറ്ററിൽ  റിലീസ് ആവുന്ന  തീയറ്റർ പ്രിന്റ്  മാത്രമേ ലഭിക്കുകയുള്ളൂ 🗑️\n\n</b>"
